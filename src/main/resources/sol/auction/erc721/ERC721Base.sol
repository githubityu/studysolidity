...
contract ERC721Base is AssetRegistryStorage, IERC721Base, ERC165 {
   ...
   */
  function ownerOf(uint256 assetId) override external view returns (address) {
  ...
  function getApprovedAddress(uint256 assetId) override external view returns (address) {
    return _getApprovedAddress(assetId);
  }
   ...
   * @return bool true if the asset has been approved by the holder
   */
  function isAuthorized(address operator, uint256 assetId) override external view returns (bool) {
   ...
   */
  function setApprovalForAll(address operator, bool authorized) override external {
   ...
   */
  function approve(address operator, uint256 assetId) override external {
   ...
   */
  function safeTransferFrom(address from, address to, uint256 assetId) override external {
   ...
   */
  function safeTransferFrom(address from, address to, uint256 assetId, bytes memory userData) override external {
   ...
   * @param assetId uint256 ID of the asset to be transferred
   */
  function transferFrom(address from, address to, uint256 assetId) override external {
